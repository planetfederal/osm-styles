...
for ysldfile in *.ysld; do

  # strip the extension from the filename to use for layer/style names
  layername=`basename $ysldfile .ysld`
  ...
  # create an empty style object in the workspace, using the same name
  curl -v -u $login -XPOST -H "Content-type: text/xml" \
    -d "<style><name>$layername</name><filename>$ysldfile</filename></style>" \
    $restapi/workspaces/$workspace/styles
  ...
  curl -v -u $login -XPOST -d@layergroup.xml -H "Content-type: text/xml" http://localhost:8080/geoserver/rest/layergroups
